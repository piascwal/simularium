...
import type { Agent, FoodSource, Exit, Alarm, Door } from '../core/agent';
...
// Teste dans l'ordre agent -> refuge -> obstacle -> porte -> nourriture -> sortie -> alarme,
...
export function eraseAt(state: EraseState, zoneScale: number, x: number, y: number): void {
  const { agents, obstacles, doors, food, exits, alarms } = state;
  let bd = Infinity, bi = -1;
  agents.forEach((a, i) => {
    const d = Math.hypot(a.x - x, a.y - y);
    if (d < bd) { bd = d; bi = i; }
  ...
  for (let i = 0; i < doors.length; i++) {
    const cp = closestPointOnWall(x, y, doors[i].points);
    if (Math.hypot(cp.x - x, cp.y - y) < doors[i].thickness) { doors.splice(i, 1); return; }
  }
